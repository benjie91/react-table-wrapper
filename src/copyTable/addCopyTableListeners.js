/* Create array of selected body rows */
const getSelectedBodyRows = (body, allRowState) => {
...
const createBodyText = (body, checkedColumns, allRowState) => {
  // BR stands for "Body Row"
  // SBR stands for "Selected BR"
  const allSBR = getSelectedBodyRows(body, allRowState);
...
const addCopyTableListeners = (tableID, pasteStyle) => {
  ...
  // get table and its button
  // assign event listeners to the table and button
  const table = document.getElementById(tableID);
  const btn = table.getElementsByTagName("button")[0];

  btn.addEventListener("click", () => {
    // copies cells when Copy Seelcted Cells button is clicked
    executeCopy(table);
  });
  table.addEventListener("copy", () => {
    // copies cells when Command-C is used
